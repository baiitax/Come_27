    ...
    <div className="admin-shell relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10">
      ...
      <div className="glass relative w-full max-w-[420px] rounded-3xl p-8 shadow-[0_24px_80px_rgba(16,24,40,0.12)] md:p-10">
        <div className="mb-8 text-center">
          ...
          <h1 className="font-display text-2xl font-extrabold tracking-tight text-[#172033]">Reset your password</h1>
          <p className="mt-1.5 text-sm text-[#667085]">Enter your account email and we&apos;ll send you a secure reset link.</p>
        </div>

        {sent ? (
          <div className="rounded-xl border border-[rgba(18,183,106,0.35)] bg-[rgba(18,183,106,0.08)] px-4 py-3.5 text-sm leading-relaxed text-[#027A48]" role="status">
            If an account exists for this email, a reset link has been sent. The link is valid for 30 minutes.
            <Link href="/admin/login" className="mt-2 block font-bold text-[#0E8A5A] hover:underline">Back to sign in →</Link>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="email" className="mb-1.5 block text-[0.72rem] font-semibold text-[#364152]">Email</label>
              <input id="email" name="email" type="email" required placeholder="admin@example.com"
                className="w-full rounded-xl border border-[rgba(16,24,40,0.1)] bg-white px-4 py-3 text-sm text-[#172033] placeholder:text-[#98A2B3] outline-none focus:border-[#0E8A5A]/50 focus:ring-4 focus:ring-[#0E8A5A]/10" />
            </div>
