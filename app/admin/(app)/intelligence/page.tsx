    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          <CardHead title="Issue Pulse" sub="Voluntary submissions by topic — current, previous period, growth, LGAs mentioning." right={<Link href="/admin/engagement/priorities" className="text-xs font-bold text-[#9C7427] hover:underline">Full engine →</Link>} />
          {pulse.length === 0 ? (
        ...
        <Card>
          ...
          {attention.length === 0 ? <EmptyState title="All clear" /> : (
            <ul className="divide-y divide-[rgba(16,24,40,0.06)]">
              {attention.map((a) => (
                <li key={a.label}>
                  <Link href={a.href} className="flex items-center justify-between gap-3 px-5 py-3 transition-colors hover:bg-[rgba(16,24,40,0.04)]">
                    <span className="flex items-center gap-2.5"><Badge tone={sevTone[a.severity] ?? 'slate'}>{a.severity}</Badge><span className="text-sm text-[#364152]">{a.label}</span></span>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-2 px-5 py-4 md:grid-cols-4">
            {lgas.map((l) => (
              <div key={l.id} className={`rounded-lg border px-3 py-2 ${l.level === 'high' ? 'border-[#0E8A5A]/40 bg-[#0E8A5A]/[0.07]' : l.level === 'medium' ? 'border-[#C9A24B]/35 bg-[#C9A24B]/[0.06]' : 'border-[rgba(16,24,40,0.08)] bg-[rgba(16,24,40,0.03)]'}`}>
                <p className="truncate text-[0.72rem] font-bold text-white">{l.name}</p>
                <p className="text-[0.58rem] uppercase tracking-wide text-[#667085]">{l.level}</p>
              </div>
            ))}
          </div>
        </Card>
