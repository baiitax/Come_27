...
// ============================================================ tokens
export const A = {
  panel: 'rounded-xl border border-white/[0.07] bg-[#12161A]',
  panelHover: 'rounded-xl border border-white/[0.07] bg-[#12161A] transition-colors hover:border-white/[0.14]',
  input:
    'w-full rounded-lg border border-white/[0.09] bg-[#0D1114] px-3.5 py-2.5 text-sm text-[#ECEDEA] placeholder:text-[#5E6A63] outline-none transition-colors focus:border-[#C9A24B]/60 focus:ring-2 focus:ring-[#C9A24B]/20',
  ...
  btnGhost:
    'inline-flex items-center justify-center gap-2 rounded-lg border border-white/[0.1] px-4 py-2.5 text-sm font-semibold text-[#C8CFC9] transition-colors hover:bg-white/[0.05] hover:text-white',
};
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] px-5 py-4">
      <div>
        ...
        {sub && <p className="mt-0.5 text-xs text-[#8A968E]">{sub}</p>}
      </div>
      {right}
    </div>
  );
...
export function Badge({
  ...
  const tones: Record<string, string> = {
    green: 'bg-[#0E8A5A]/15 text-[#4CC39A] border-[#0E8A5A]/30',
    gold: 'bg-[#C9A24B]/12 text-[#DDBE72] border-[#C9A24B]/35',
    crimson: 'bg-[#A31621]/15 text-[#E06A75] border-[#A31621]/40',
    red: 'bg-[#C0323E]/15 text-[#E06A75] border-[#C0323E]/40',
    blue: 'bg-[#3B82F6]/12 text-[#7EB0F5] border-[#3B82F6]/30',
    neutral: 'bg-white/[0.06] text-[#B9C2BB] border-white/[0.1]',
    slate: 'bg-[#5E7168]/15 text-[#9FB0A6] border-[#5E7168]/30',
  };
  return (
    ...
    <div className={`${A.panelHover} group relative h-full cursor-default p-4 ${href ? 'cursor-pointer' : ''}`}>
      <div className="flex items-center justify-between">
        ...
        {demo ? (
          ...
          <span className={`text-[0.68rem] font-bold ${change >= 0 ? 'text-[#4CC39A]' : 'text-[#E06A75]'}`}>
...
export function PageHeader({ title, sub, right, crumb }: { title: string; sub?: string; right?: React.ReactNode; crumb?: string }) {
  return (
    <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
      <div>
        {crumb && <p className="text-[0.62rem] font-bold uppercase tracking-[0.2em] text-[#C9A24B]">{crumb}</p>}
        <h1 className="font-display text-2xl font-extrabold tracking-tight text-white md:text-[1.7rem]">{title}</h1>
        {sub && <p className="mt-1 max-w-2xl text-sm text-[#8A968E]">{sub}</p>}
      </div>
...
export function Field({ label, hint, required, children, className = '' }: { label: string; hint?: string; required?: boolean; children: React.ReactNode; className?: string }) {
  return (
    <label className={`block ${className}`}>
      <span className="mb-1.5 flex items-baseline gap-2 text-[0.7rem] font-bold uppercase tracking-[0.14em] text-[#9AA39C]">
        {label}
        {required && <span className="text-[#E06A75]">*</span>}
        {hint && <span className="normal-case tracking-normal text-[#5E6A63]">{hint}</span>}
      </span>
      {children}
    </label>
  );
...
export function Check({ label, ...rest }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  return (
    <label className="flex cursor-pointer items-center gap-2.5 text-sm text-[#C8CFC9]">
      <input type="checkbox" {...rest} className="h-4 w-4 rounded border-white/20 bg-[#0D1114] accent-[#0E8A5A]" />
      {label}
    </label>
  );
}
  ...
  return (
    <div className={`overflow-x-auto ${className}`}>
      <table className="w-full min-w-[640px] text-left text-sm">
        <thead>
          <tr className="border-b border-white/[0.07] text-[0.62rem] font-bold uppercase tracking-[0.16em] text-[#8A968E]">{head}</tr>
        </thead>
...
export function Td({ children, className = '' }: { children?: React.ReactNode; className?: string }) {
  return <td className={`px-4 py-3 align-middle text-[#C8CFC9] ${className}`}>{children}</td>;
}

export function EmptyState({ title, sub }: { title: string; sub?: string }) {
  return (
    <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-white/[0.08] bg-white/[0.03] text-xl">◇</div>
      <p className="font-display text-sm font-bold text-white">{title}</p>
      {sub && <p className="mt-1 max-w-sm text-xs text-[#8A968E]">{sub}</p>}
    </div>
  );
}
...
export function DemoBanner() {
  return (
    <div className="mb-4 flex items-center gap-2 rounded-lg border border-[#C9A24B]/30 bg-[#C9A24B]/[0.07] px-4 py-2.5 text-xs text-[#DDBE72]">
...
export function FormError({ error }: { error?: string }) {
  ...
  return (
    <div className="mb-4 rounded-lg border border-[#C0323E]/40 bg-[#C0323E]/10 px-4 py-2.5 text-sm text-[#E06A75]">{error}</div>
  );
}
...
export function SaveBar({ onSave, saving, saveLabel = 'Save' }: { onSave: () => void; saving?: boolean; saveLabel?: string }) {
  ...
  return (
    <div className="sticky bottom-4 z-10 flex items-center justify-end gap-2 rounded-xl border border-white/[0.08] bg-[#12161A]/95 p-3 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] backdrop-blur">
